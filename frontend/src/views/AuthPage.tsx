...
export function AuthPage() {
  ...
  const [displayMode, setDisplayMode] = useState<"login" | "register">(isRegister ? "register" : "login");
  const [slideDir, setSlideDir] = useState<"left" | "right">("left");

  // Sync with URL changes
  useEffect(() => {
    const target = isRegister ? "register" : "login";
    if (target !== displayMode && !animating) {
      setSlideDir(target === "register" ? "left" : "right");
      setAnimating(true);
      setTimeout(() => {
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-64px)] items-center justify-center px-4 py-12 overflow-hidden">
      {/* ══════════ Fullscreen rotating background ══════════ */}
      <div className={`pointer-events-none absolute inset-0 overflow-hidden transition-opacity duration-[1200ms] ease-out ${mounted ? "opacity-100" : "opacity-0"}`}>
        {BG_IMAGES.map((src, i) => (
          <div
            key={src}
      ...
      {/* ══════════ Centered card ══════════ */}
      <div className={`relative w-full max-w-md transition-all duration-700 ease-out ${mounted ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-[0.97]"}`}>
            ...
            {displayMode === "login" ? (
              ...
              <div>
                ...
                <p className="text-center text-sm text-surface-400">
                  Noch kein Konto?{" "}
                  <button onClick={() => switchTo("register")} className="font-semibold text-accent-400 hover:text-accent-300 transition-colors">
                    Jetzt registrieren
                  </button>
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-center text-sm text-surface-400">
                  Bereits registriert?{" "}
                  <button onClick={() => switchTo("login")} className="font-semibold text-accent-400 hover:text-accent-300 transition-colors">
                    Jetzt anmelden
                  </button>
                </p>
              </div>
